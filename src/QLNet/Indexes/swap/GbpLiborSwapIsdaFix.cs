    ...
    public class GbpLiborSwapIsdaFix : SwapIndex
    {
        ...
        public GbpLiborSwapIsdaFix(Period tenor, Handle<YieldTermStructure> h)
            : base("GbpLiborSwapIsdaFix", // familyName
                ...
                new GBPCurrency(),
                new TARGET(),
                tenor > new Period(1, TimeUnit.Years) ? new Period(6, TimeUnit.Months) : new Period(1, TimeUnit.Years), // fixedLegTenor
                BusinessDayConvention.ModifiedFollowing, // fixedLegConvention
                new Actual365Fixed(), // fixedLegDaycounter
                tenor > new Period(1, TimeUnit.Years) ? new GBPLibor(new Period(6, TimeUnit.Months), h) : new GBPLibor(new Period(3, TimeUnit.Months), h))
        {
        }
    }
}
