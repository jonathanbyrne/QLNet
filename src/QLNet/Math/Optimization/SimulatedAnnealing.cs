...
namespace QLNet.Math.Optimization
{
    ...
    public class SimulatedAnnealing<RNG> : OptimizationMethod where RNG : class, IRNGTraits, new()
    {
        ...
        public SimulatedAnnealing(double lambda, double T0, double epsilon, int m, RNG rng = default)
        {
        ...
        /// <param name="rng"></param>
        public SimulatedAnnealing(double lambda, double T0, int K, double alpha, RNG rng = default)
        {
        ...
        public override EndCriteria.Type minimize(Problem P, EndCriteria endCriteria)
        {
            ...
            do
            {
                ...
                do
                {
                    ...
                    ilo_ = 0;
                    ihi_ = 1;
                    ynhi_ = values_[0] + tt_ * System.Math.Log(rng_.next().value);
                    ylo_ = ynhi_;
                    yhi_ = values_[1] + tt_ * System.Math.Log(rng_.next().value);
                    if (ylo_ > yhi_)
                    {
                        ihi_ = 0;
                        ilo_ = 1;
                    ...
                    for (i_ = 2; i_ < n_ + 1; i_++)
                    {
                        yt_ = values_[i_] + tt_ * System.Math.Log(rng_.next().value);
                        if (yt_ <= ylo_)
                        {
                            ilo_ = i_;
                            ylo_ = yt_;
        ...
        protected void amotsa(Problem P, double fac)
        {
            ...
            yflu_ = ytry_ - tt_ * System.Math.Log(rng_.next().value);
            if (yflu_ < yhi_)
            {
                values_[ihi_] = ytry_;
                yhi_ = yflu_;
