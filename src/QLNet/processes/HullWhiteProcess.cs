...
namespace QLNet.processes
{
    ...
    public class HullWhiteProcess : StochasticProcess1D
    {
        ...
        {
            double alpha_drift = sigma_ * sigma_ / (2 * a_) * (1 - System.Math.Exp(-2 * a_ * t));
            double shift = 0.0001;
    ...
    public class HullWhiteForwardProcess : ForwardMeasureProcess1D
    {
        ...
        public double M_T(double s, double t, double T)
        {
            if (a_ > Const.QL_EPSILON)
            {
                ...
                double exp1 = System.Math.Exp(-a_ * (t - s));
                double exp2 = System.Math.Exp(-a_ * (T - t));
                double exp3 = System.Math.Exp(-a_ * (T + t - 2.0 * s));
