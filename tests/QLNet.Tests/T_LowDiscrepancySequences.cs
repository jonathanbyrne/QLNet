...
namespace QLNet.Tests
{
    ...
    [JetBrains.Annotations.PublicAPI] public class T_LowDiscrepancySequences
    {
        ...
        public void testHalton()
        {
            ...
            points = (int)System.Math.Pow(2.0, 5) - 1;  // five cycles
            for (i = 0; i < points; i++)
            {
                point = rsg.nextSequence().value;
                var error = System.Math.Abs(point[0] - vanderCorputSequenceModuloTwo[i]);
            ...
            dimensionality = 2;
            rsg = new HaltonRsg(dimensionality, 0, false, false);
            points = (int)System.Math.Pow(3.0, 3) - 1;  // three cycles of the higher dimension
            for (i = 0; i < points; i++)
            {
                point = rsg.nextSequence().value;
                var error = System.Math.Abs(point[0] - vanderCorputSequenceModuloTwo[i]);
                ...
                error = System.Math.Abs(point[1] - vanderCorputSequenceModuloThree[i]);
                if (error > tolerance)
                {
            ...
            {
                // five cycle
                points = (int)System.Math.Pow(2.0, j) - 1;  // base 2
                for (; k < points; k++)
                {
                    point = rsg.nextSequence().value;
                    stat.add(point);
            ...
            {
                // three cycle
                points = (int)System.Math.Pow(3.0, j) - 1;  // base 3
                for (; k < points; k++)
                {
                    point = rsg.nextSequence().value;
                    stat.add(point);
        ...
        internal void testGeneratorDiscrepancy(IRNGFactory generatorFactory, double[][] discrepancy)
        {
            ...
            for (var i = 0; i < 8; i++)
            {
                ...
                for (j = jMin; j < jMin + sampleLoops; j++)
                {
                    ...
                    if (System.Math.Abs(discr - discrepancy[i][j - jMin]) > tolerance * discr)
                    {
