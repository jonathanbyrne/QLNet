...
namespace QLNet.Tests
{
    ...
    [JetBrains.Annotations.PublicAPI] public class T_PSACurve
    {
        [Fact]
        public void testCashedValues()
        {

            var startDate = new Date(01, 03, 2007);
            var period = new Period(360, TimeUnit.Months);
            Calendar calendar = new TARGET();
            var endDate = calendar.advance(startDate, period, BusinessDayConvention.Unadjusted);

            var schedule = new Schedule(startDate, endDate, new Period(1, TimeUnit.Months), calendar,
