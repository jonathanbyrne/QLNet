...
namespace QLNet.Tests
{
    ...
    public class T_ConvertibleBond
    {
        ...
        public void testBond()
        {
            ...
            error = System.Math.Abs(amZero.NPV() - zero.settlementValue());
            if (error > tolerance)
            {
                QAssert.Fail("failed to reproduce zero-coupon bond price:"
            ...
            error = System.Math.Abs(euFixed.NPV() - fixedBond.settlementValue());
            if (error > tolerance)
            {
            ...
            error = System.Math.Abs(amFixed.NPV() - fixedBond.settlementValue());
            if (error > tolerance)
            {
                QAssert.Fail("failed to reproduce fixed-coupon bond price:"
            ...
            error = System.Math.Abs(euFloating.NPV() - floating.settlementValue());
            if (error > tolerance)
            {
            ...
            error = System.Math.Abs(amFloating.NPV() - floating.settlementValue());
            if (error > tolerance)
            {
                QAssert.Fail("failed to reproduce floating-rate bond price:"
